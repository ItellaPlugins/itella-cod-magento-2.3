...
class Totals extends AbstractTotal
{
    ...
    const TOTAL_CODE = 'itella_cod_fee';
    
    const BASE_TOTAL_CODE = 'base_itella_cod_fee';
    ...
    {
        $this->feeType = (float)$scopeConfig->getValue('payment/itella_cod/cod_fee_type', ScopeInterface::SCOPE_STORE);
    ...
    public function canAddFee(Quote $quote){
        ...
        if ((count($paymentMethodsList) == 1) && (current($paymentMethodsList)->getCode() == 'itella_cod')) {
            return true;
        }
        return ($quote->getPayment()->getMethod() == 'itella_cod');
    }
    ...
    private function getFee(Quote $quote){
        $this->fee = $quote->getItellaCodFee();
